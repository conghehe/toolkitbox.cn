...
const TOOL_SITE_DIR = 'D:/AI/workbuddy_space/tmp_task/2026-06-22-16-36-47/outputs/toolsite';
const OUTPUT_HTML = 'D:/AI/workbuddy_space/tmp_task/2026-06-22-16-36-47/outputs/toolkitbox.cn/index.html';

// 读取所有工具目录
const tools = fs.readdirSync(TOOL_SITE_DIR).filter(f => {
  const fullPath = path.join(TOOL_SITE_DIR, f);
...
// 生成工具卡片
const toolCards = tools.map(toolDir => {
  const iconInfo = iconMap[toolDir] || { icon: '🔧', bg: '#f0edff', color: '#6c5ce7', cat: 'dev' };
  
  // 读取工具 HTML 获取标题和描述
  let title = toolDir;
  let desc = '工具描述';
  try {
    const toolHtmlPath = path.join(TOOL_SITE_DIR, toolDir, 'index.html');
  ...
  return `<a href="/${toolDir}/" class="tool-card" data-cat="${iconInfo.cat}"><div class="card-icon" style="background:${iconInfo.bg};color:${iconInfo.color}">${iconInfo.icon}</div><h3>${title}</h3><p class="desc">${desc}</p><div class="tags"><span class="tag">${toolDir.split('-')[0]}</span></div></a>`;
}).join('');
...
<head>
  ...
  <meta name="description" content="免费在线开发者工具集，${tools.length}款工具，数据本地处理，安全不上传">
...
<body>
  <div class="hero">
    ...
    <div class="stats">
      <div class="stat"><div class="stat-num">${tools.length}</div><div class="stat-label">在线工具</div></div>
...
console.log(`✅ 生成完成: ${tools.length} 个工具卡片`);
